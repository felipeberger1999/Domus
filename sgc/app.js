...
function renderPrevisao(){
  ...
  <div class="grid">
   ...
   <div class="card span-7"><h3>Receitas × Despesas — realizado e projetado</h3>
     ...
     <div style="display:flex;align-items:flex-end;gap:6px;height:168px;padding-top:6px">${todas.map(t=>`<div style="flex:1;display:flex;flex-direction:column;justify-content:flex-end;align-items:center;height:100%;opacity:${t.tipo==='proj'?.55:1}"><div style="display:flex;gap:2px;align-items:flex-end;justify-content:center;width:100%;height:100%"><div style="width:7px;background:var(--pinho);border-radius:3px 3px 0 0;height:${Math.max(t.rec/maxV*100,1)}%" title="Receita ${brl(t.rec)}"></div><div style="width:7px;background:var(--terracota);border-radius:3px 3px 0 0;height:${Math.max(t.desp/maxV*100,1)}%" title="Despesa ${brl(t.desp)}"></div></div></div>`).join('')}</div>
     <div style="display:flex;gap:6px;margin-top:6px">${todas.map(t=>`<div style="flex:1;text-align:center;font-size:10px;color:var(--musgo)">${t.mes}</div>`).join('')}</div></div>

   <div class="card span-5"><h3>Saldo em caixa projetado</h3>
     <div style="display:flex;align-items:flex-end;gap:6px;height:168px;padding-top:6px">${todas.map(t=>`<div style="flex:1;display:flex;flex-direction:column;justify-content:flex-end;align-items:center;gap:3px;height:100%;opacity:${t.tipo==='proj'?.55:1}"><span style="font-size:9px;font-weight:600">${(t.saldo/1000).toFixed(0)}k</span><div style="width:62%;max-width:24px;background:var(--pinho);border-radius:4px 4px 0 0;height:${Math.max(t.saldo/maxS*100,1)}%"></div></div>`).join('')}</div>
     <div style="display:flex;gap:6px;margin-top:6px">${todas.map(t=>`<div style="flex:1;text-align:center;font-size:10px;color:var(--musgo)">${t.mes}</div>`).join('')}</div></div>
