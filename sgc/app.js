...
function makeSeed(){
  ...
  const regua=[
    {ordem:1,rotulo:'Lembrete amigável',off:-3,canal:'whatsapp',canais:['whatsapp','email'],tom:'Cordial',publico:'Cotas a vencer (3 dias)',msg:'Sua cota vence em 3 dias. 🏛️',ativo:true},
    {ordem:2,rotulo:'Aviso no vencimento',off:0,canal:'whatsapp',canais:['whatsapp','email'],tom:'Cordial',publico:'Cotas que vencem hoje',msg:'Sua cota vence hoje. Pague pelo PIX.',ativo:true},
    {ordem:3,rotulo:'1º aviso de atraso',off:1,canal:'whatsapp',canais:['whatsapp'],tom:'Cordial e firme',publico:'Inadimplentes (1 dia)',msg:'Cota venceu ontem. Regularize para evitar encargos.',ativo:true},
    {ordem:4,rotulo:'2ª via + encargos',off:7,canal:'email',canais:['email','whatsapp'],tom:'Formal e cordial',publico:'Inadimplentes (7 dias)',msg:'Cota em aberto há 7 dias. Segue 2ª via com encargos.',ativo:true},
    {ordem:5,rotulo:'Cobrança formal',off:15,canal:'email',canais:['email','whatsapp'],tom:'Firme',publico:'Inadimplentes (15 dias)',msg:'Débito em aberto há 15 dias. Regularização em 5 dias.',ativo:true},
    {ordem:6,rotulo:'Notificação extrajudicial',off:30,canal:'email',canais:['email'],tom:'Formal',publico:'Inadimplentes (30 dias)',msg:'Notificação extrajudicial de débito.',ativo:true}];
  const funcionarios=[
...
function salvarRegua(ev){ev.preventDefault();const prev=DATA.regua.slice();const rows=[...document.querySelectorAll('#regua-rows .etapa-edit')];DATA.regua=rows.map(r=>{const rot=r.querySelector('.er-rot').value.trim()||'Etapa';const old=prev.find(o=>o.rotulo===rot)||{};const canal=r.querySelector('.er-canal').value;return{rotulo:rot,off:parseInt(r.querySelector('.er-off').value)||0,canal,canais:old.canais||[canal],tom:old.tom||'',publico:r.querySelector('.er-pub').value.trim(),msg:r.querySelector('.er-msg').value.trim(),assunto:old.assunto||'',email:old.email||'',wpp:old.wpp||'',ativo:r.querySelector('.er-ativo').checked};}).sort((a,b)=>a.off-b.off);DATA.regua.forEach((e,i)=>e.ordem=i+1);addEvent('Cobranca','Régua atualizada',`${DATA.regua.length} etapa(s) configurada(s) pelo gestor`);fecharRegua();render();return false;}
...
function _qSGC(s){return String(s==null?'':s).replace(/\\/g,'\\\\').replace(/'/g,"\\'");}
function extratoFornecedorSGC(fornecedor){
  const ls=DATA.contasPagar.filter(c=>c.fornecedor===fornecedor).slice().sort((a,b)=>a.vencimento<b.vencimento?1:-1);
  if(!ls.length)return;
  const pago=sum(ls.filter(c=>c.status==='paga'),c=>c.total), aberto=sum(ls.filter(c=>c.status==='pendente'||c.status==='aprovada'),c=>c.total), total=sum(ls,c=>c.total);
  const rows=ls.map(c=>`<tr><td style="white-space:nowrap"><strong>${c.numero}</strong></td><td style="white-space:nowrap">${dataBR(c.pago_em||c.vencimento)}</td><td>${c.descricao}<div class="muted" style="font-size:11px">${mlabel(c.competencia)} · ${c.grupo} › ${c.conta}</div></td><td class="num"><strong>${brl(c.total)}</strong></td><td><span class="badge ${c.status}">${c.status}</span></td></tr>`).join('');
  document.querySelector('#modal-razao .rz-title').textContent='Extrato do fornecedor';
  document.querySelector('#modal-razao .rz-sub').innerHTML=`<b>${fornecedor}</b> · histórico de lançamentos · total <strong>${brl(total)}</strong> · pago <strong>${brl(pago)}</strong> · em aberto <strong style="color:var(--terracota)">${brl(aberto)}</strong>`;
  document.querySelector('#modal-razao .rz-body').innerHTML=`<div class="tblx"><table class="tbl" style="min-width:520px"><thead><tr><th>Nº</th><th>Data</th><th>Descrição</th><th class="num">Valor</th><th>Status</th></tr></thead><tbody>${rows}</tbody></table></div>`;
  document.getElementById('modal-razao').classList.add('open');
}
function _inssSGC(b){const t=[[1412,0.075],[2666.68,0.09],[4000.03,0.12],[8157.41,0.14]];let prev=0,val=0;for(let i=0;i<t.length;i++){const base=Math.min(b,t[i][0]);if(base>prev){val+=(base-prev)*t[i][1];prev=t[i][0];}if(b<=t[i][0])break;}return Math.round(val*100)/100;}
function fichaFuncSGC(fid){
  const f=DATA.funcionarios.find(z=>z.id===fid); if(!f)return;
  const comps=['2026-01','2026-02','2026-03','2026-04','2026-05','2026-06'];
  const heVal=Math.round((f.salario/220)*1.5*(f.horasExtras||0)*100)/100;
  const rows=comps.slice().reverse().map((c,i)=>{const he=i===0?heVal:0;const bruto=f.salario+he;const inss=_inssSGC(bruto);const liq=Math.round((bruto-inss)*100)/100;return `<tr><td style="white-space:nowrap">${mlabel(c)}</td><td>Folha mensal${he?` <span class="muted">+ ${f.horasExtras}h extras</span>`:''}</td><td class="num">${brl(bruto)}</td><td class="num">${brl(inss)}</td><td class="num"><strong>${brl(liq)}</strong></td></tr>`;}).join('');
  const fgts=Math.round(f.salario*0.08*100)/100;
  document.querySelector('#modal-razao .rz-title').textContent='Ficha · '+f.nome;
  document.querySelector('#modal-razao .rz-sub').innerHTML=`${f.cargo} · CPF ${f.cpf} · admissão ${dataBR(f.admissao)} · ${f.jornada} · <span class="badge ${f.status}">${f.status.replace('_',' ')}</span> · FGTS/mês <strong>${brl(fgts)}</strong>`;
  document.querySelector('#modal-razao .rz-body').innerHTML=`<div class="tblx"><table class="tbl" style="min-width:520px"><thead><tr><th>Competência</th><th>Lançamento</th><th class="num">Bruto</th><th class="num">INSS (est.)</th><th class="num">Líquido (est.)</th></tr></thead><tbody>${rows}</tbody></table></div><p class="muted" style="font-size:11.5px;margin-top:10px">Demonstrativo com valores estimados (INSS por faixa). A folha oficial é processada pelo agente de DP no eSocial.</p>`;
  document.getElementById('modal-razao').classList.add('open');
}
...
function renderDP(){
  ...
  const quadro = `<div class="card span-12"><h3>Quadro de funcionários <span class="r">${ativos.length} ativos na folha</span></h3><table class="tbl"><thead><tr><th>Nome</th><th>Cargo</th><th>Admissão</th><th>Jornada</th><th class="num">Salário</th><th class="num">H. extras</th><th>Status</th><th class="num">Ação</th></tr></thead><tbody>${f.map(x=>`<tr><td><a class="lnk" onclick="fichaFuncSGC(${x.id})" title="Ver ficha e folha"><strong>${x.nome}</strong></a><div class="muted" style="font-size:11px">CPF ${x.cpf}</div></td><td>${x.cargo}</td><td class="muted">${dataBR(x.admissao)}</td><td class="muted">${x.jornada}</td><td class="num">${brl(x.salario)}</td><td class="num">${x.horasExtras||0}h</td><td><span class="badge ${x.status}">${x.status.replace('_',' ')}</span></td><td class="num" style="white-space:nowrap"><button class="btn sm" onclick="fichaFuncSGC(${x.id})">Ficha</button> ${x.status==='ativo'?`<button class="btn sm" onclick="iniciarDemissao(${x.id})">Demissão</button>`:'<span class="muted">em andamento</span>'}</td></tr>`).join('')}</tbody></table></div>`;
